...
function [Pp , Para, Xref ,X, RMS]= Helmert2D_RANSAC(Xref,X)
...
NP = size(Xref,1); % number of point
...
er = Err(1:size(Err,1)/2).^2 +Err(size(Err,1)/2+1:size(Err,1)).^2;
er_ = sort(er,'descend');
len = min(size(er_,1),4);
RMS = sum(er_(1:len))/size(er_(1:len),1);
