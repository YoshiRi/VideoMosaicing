...
refnumMap = peakMap;
Data = csvread('../../../Phase-Correlation/imregpoc/MakeDataSet/626EXP_FP.csv')
%Data = csvread('../../../Phase-Correlation/imregpoc/MakeDataSet/626EXP_FP.csv')
...
len = size(Data,2);
pmat = Data(len*4+1:len*5,:); % get map 
valMap = zeros(len,len,4);
valMap(:,:,1) = Data(len*0+1:len*1,:);
valMap(:,:,2) = Data(len*1+1:len*2,:);
valMap(:,:,4) = -Data(len*2+1:len*3,:)/pi*180; %theta
valMap(:,:,3) = Data(len*3+1:len*4,:); %kappa 
...
%% save the result
save('Optimized_626_python','T_val','nT_val','time');